...
CREATE_URL="https://raw.githubusercontent.com/lupsys/AUTO-Debian-KVM/main/create-debian-vm2.sh"
...
select opt in "${options[@]}"; do
  ...
  "Instalar dependencias (GitHub)")
    ...
    ;;
  "Instalar VM (local)")
    if [ -f "create-debian-vm2.sh" ]; then
      sudo bash create-debian-vm2.sh
    else
      echo "Error: create-debian-vm2.sh no existe localmente."
    fi
    break
    ;;
  "Instalar VM (GitHub)")
    echo "--> Descargando create-debian-vm2.sh..."
